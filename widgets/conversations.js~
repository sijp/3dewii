 ...
 */
ConversationsWidget.prototype = new GamlibaGrabbableWidget();
...
ConversationsWidget.prototype.init = function(geometry)
{
	...
	Gamliba.getInstance().scene.add(this.bboxMeshHelper);
...
ConversationsWidget.prototype.refresh = function()
{
	...
	topright = toScreenXY(this.bboxMeshHelper.position.clone().addSelf(boundingbox.max),Gamliba.getInstance().camera,Gamliba.getInstance().renderer.domElement);
	bottomleft = toScreenXY(this.bboxMeshHelper.position.clone().addSelf(boundingbox.min),Gamliba.getInstance().camera,Gamliba.getInstance().renderer.domElement);
